...
export default function CreateOfferPage() {
  ...
  const prefilledDonationId = urlParams.get('donation') || '';
  ...
  // Helyszín: megye + szabad szöveges város
  ...
  }, [prefilledDonationId]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  async function handleImageUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    for (const file of files.slice(0, 3 - images.length)) {
      ...
      const path = `${user!.id}/offers/${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`;
  ...
  function buildLocation(): string {
    const parts = [locationCity.trim(), locationCounty].filter(Boolean);
    return parts.join(', ');
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!title.trim() || !description.trim()) {
      showToast('error', 'Cím és leírás kötelező'); return;
    }
    setLoading(true);
    ...
    setLoading(false);
    if (error) {
      showToast('error', 'Hiba történt, próbáld újra');
    } else {
      showToast('success', 'Felajánlásod sikeresen közzétéve!');
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="glass rounded-2xl p-5 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            {([
              { v: 'item' as OfferType, icon: Package, label: 'Tárgy', sub: 'Játék, ruha, bútor, élelmiszer...' },
              { v: 'service' as OfferType, icon: Wrench, label: 'Szolgáltatás', sub: 'Fuvar, arcfestés, szerelés...' },
            ] as const).map(({ v, icon: Icon, label, sub }) => (
              <button key={v} type="button" onClick={() => setOfferType(v)}
        ...
        <div className="glass rounded-2xl p-5 space-y-3">
          <div className="flex items-center gap-2">
            ...
            <p className="text-xs font-semibold uppercase tracking-widest text-zinc-500">Helyszín <span className="normal-case text-zinc-600 font-normal">(opcionális)</span></p>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs text-zinc-400 mb-1.5 block">Megye</label>
            ...
            <div>
              <label className="text-xs text-zinc-400 mb-1.5 block">Város / település</label>
              <input type="text" value={locationCity} onChange={(e) => setLocationCity(e.target.value)}
                placeholder="pl. Érd, Esztergom..."
                className="w-full px-3 py-2.5 glass-input rounded-xl text-zinc-100 placeholder-zinc-500 focus:outline-none text-sm" />
            </div>
          </div>
          {(locationCity || locationCounty) && (
            <p className="text-xs text-zinc-500 flex items-center gap-1">
              ...
              {[locationCity.trim(), locationCounty].filter(Boolean).join(', ')}
            </p>
          )}
        </div>
        ...
        <div className="glass rounded-2xl p-5 space-y-3">
          <p className="text-xs font-semibold uppercase tracking-widest text-zinc-500">
            Képek <span className="normal-case text-zinc-600 font-normal">(max 3, opcionális)</span>
          </p>
          <div className="flex gap-3 flex-wrap">
            {images.map((url, i) => (
              <div key={i} className="relative w-24 h-24 rounded-xl overflow-hidden bg-zinc-800">
                <img src={url} alt="" className="w-full h-full object-cover" />
                <button type="button" onClick={() => setImages((p) => p.filter((_, idx) => idx !== i))}
                  className="absolute top-1 right-1 w-5 h-5 bg-black/80 rounded-full flex items-center justify-center text-zinc-300 hover:text-white">
                  <X className="w-3 h-3" />
                </button>
              </div>
            ))}
            {images.length < 3 && (
              <label className={`w-24 h-24 rounded-xl border-2 border-dashed flex flex-col items-center justify-center cursor-pointer transition-colors ${uploading ? 'border-teal-500/40 opacity-50' : 'border-white/10 hover:border-teal-500/40'}`}>
                {uploading ? (
        ...
        <div className="glass rounded-2xl p-5 space-y-3">
          <div className="flex items-center justify-between gap-3">
            ...
            <button
              type="button"
              onClick={() => setLinkToDonation((v) => !v)}
              role="switch"
              aria-checked={linkToDonation}
              className={`relative flex-shrink-0 w-11 h-6 rounded-full transition-colors duration-200 focus:outline-none ${linkToDonation ? 'bg-teal-500' : 'bg-zinc-700'}`}
            >
              <span className={`absolute top-1 left-1 w-4 h-4 rounded-full bg-white shadow transition-transform duration-200 ${linkToDonation ? 'translate-x-5' : 'translate-x-0'}`} />
            </button>
          </div>
                    ...
                    <div className="space-y-1.5 max-h-48 overflow-y-auto">
                      {donationResults.map((d) => (
                        <button key={d.id} type="button"
                          onClick={() => { setSelectedDonation(d); setDonationSearch(''); setDonationResults([]); }}
