...
export default function WorldMobileDock({
  ...
  return (
    <nav className="world-mobile-dock md:hidden fixed bottom-0 left-0 right-0 z-50 safe-area-pb">
      <button
        type="button"
        ...
        {PRIMARY_WORLD_ZONES.map((zone) => {
          ...
          return (
            <button
              ...
              className={`world-dock-item flex-1 flex flex-col items-center justify-center gap-0.5 min-w-0 px-0.5 ${active ? 'world-dock-item-active' : ''}`}
            ...
            >
              ...
              />
              <span className="text-[8px] sm:text-[9px] font-semibold truncate max-w-full">{zone.label}</span>
            </button>
          );
        })}
      </div>
